...
def baseline_hy_train(trn_df_fanme, save_dir):
    ...
    train_out = cfdna_covid19.b_hyopt_train(trn_df, param_space=lgb_space)
    ...
    with open(os.path.join(save_dir, '21_bl_opt_params-auc-false.json'),'w') as f:
        json.dump(opt_params, f)
        

def baseline_retrain(trn_df_fanme, tst_df_fname,rand_id_split_fname,opt_params_fname, save_dir):
    ...
    cfdna_covid19.b_repeat_perf(trn_df,tst_df,rand_id_split_fname, opt_params, 
                                saved_fname=os.path.join(save_dir, '22_bl_entire_feat_retrain-auc-false.pkl'))
    
    print('retrain done')
    
    
def baseline_retrain_prefeature(trn_df_fanme, tst_df_fname,rand_id_split_fname,opt_params_fname, save_dir):
    ...
    for feat_idx in range(len(features_ordered)):
        ...
        i_re = cfdna_covid19.b_repeat_perf(trn_df,tst_df,rand_id_split_fname, opt_params, 
                                           saved_fname=None, is_shap=False, feat_ls=feat_ls)
        ress.append((feat_ls, i_re))

    with open(os.path.join(save_dir, '24_bl_prefeature_performance-false.pkl'), 'wb') as f:
            pickle.dump(ress, f) 
