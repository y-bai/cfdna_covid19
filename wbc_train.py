...
def wbc_hy_train(trn_df_fanme, save_dir):
    ...
    lgb_space = {
        ...
        'wbc_alpha': hp.quniform('wbc_alpha', 1.0,4.0,0.1),
        
        # 'min_gain_to_split': hp.quniform('min_gain_to_split', 0.1, 5, 0.01), # or min_split_gain
    }
    
    train_out = cfdna_covid19.wbc_hyopt_train(trn_df, param_space=lgb_space)
    ...
    with open(os.path.join(save_dir, '41_wbc_opt_params-auc.json'),'w') as f:
        json.dump(opt_params, f)
        

def wbc_retrain(trn_df_fanme,tst_df_fname, rand_id_split_fname,opt_params_fname, save_dir):
    ...
    cfdna_covid19.wbc_repeat_perf(trn_df, tst_df, rand_id_split_fname, opt_params, 
                                saved_fname=os.path.join(save_dir, '42_wbc_entire_feat_retrain-auc.pkl'))
    
    print('retrain done')


def wbc_retrain_prefeature(trn_df_fanme, tst_df_fname,rand_id_split_fname,opt_params_fname, save_dir):
    ...
    feat_imp_shap_re_df = cfdna_covid19.feat_importance_shap(os.path.join(save_dir, '42_wbc_entire_feat_retrain-auc.pkl'))
    
    feat_imp_shap_re_df.to_csv(os.path.join(save_dir, '43_wbc_feature_importance_shap.csv'), sep='\t', index=False)
    ...
    for feat_idx in range(len(features_ordered)):
        ...
        i_re = cfdna_covid19.wbc_repeat_perf(trn_df,tst_df,rand_id_split_fname, opt_params, 
                                           saved_fname=None, is_shap=False, feat_ls=feat_ls)
        ress.append((feat_ls, i_re))

    with open(os.path.join(save_dir, '44_wbc_prefeature_performance.pkl'), 'wb') as f:
            pickle.dump(ress, f) 
